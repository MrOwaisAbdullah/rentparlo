...
function SellerMarqueeCard({ seller }: { seller: Seller }) {
  ...
  return (
    <Link href={`/seller/${profile.username}`} className="block">
      <Card className="w-64 mx-2 hover:shadow-md transition-shadow">
...
export default function TopSellers({ sellers }: TopSellersProps) {
  ...
  return (
    <div className="relative flex w-full flex-col items-center justify-center gap-1 overflow-hidden py-8">
      {/* Marquee moving left to right (default) - faster speed */}
      ...
      {/* Marquee moving right to left (reverse) - faster speed */}
      <Marquee pauseOnHover reverse repeat={5} className="[--duration:20s]">
